...
本仓库也包含前置项目认知 skill：`module-deep-research`。它用于让 Agent 持续研究模块，沉淀 `.sdd/project-understanding/` 下的模块认知资产，并在后续运行中基于 git diff 增量刷新已有理解。

通用文档交付前，可使用 `document-writing-guidelines` 检查文档是否掺入了不该出现的对话过程、试错过程或个案现场，并审视术语、边界、证据和表达是否适合该文档类型。
...
```text
使用 $document-writing-guidelines 检查并改写这份文档，去除不该出现的过程污染，收敛术语，并保持内容符合文档类型。
```
...
| Skill | 何时使用 | 主要输出 |
|-------|----------|----------|
| `document-writing-guidelines` | 需要起草、修改或审视各类文档成果物时 | 目的清楚、过程污染少、术语克制的文档 |
