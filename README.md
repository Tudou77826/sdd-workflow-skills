...
- 正式《{AR编号}-{需求短名}-{模块名}模块详细设计说明书.md》：最终交付物和唯一开发依据。
- `{同名前缀}.context.md`：过程文档，只记录证据、检索过程、推导、采样和复检留痕。
...
```text
使用 $sdd-module-detailed-design-flow 针对 {模块名} 完成 {需求/AR/功能点} 的模块详细设计，维护正式说明书和同名前缀 .context.md，并在门禁通过后给出是否可进入 AICoding 的结论。
```

如果只需要某个阶段，也可以直接使用原子 skill：

```text
使用 $sdd-module-asis-analysis 分析 {模块名} 与 {需求/AR} 相关的 ASIS，正式说明书写关键结论，.context.md 写证据和检索过程。
```

```text
使用 $sdd-module-tobe-design 基于正式说明书中的 ASIS 关键结论继续设计 TOBE，确保开发必需内容写入正式说明书。
```
...
| Skill | 何时使用 | 主要输出 |
|-------|----------|----------|
| `sdd-module-detailed-design-flow` | 需要完整串联 ASIS、TOBE、Gate，或进行演练验证时 | 统一维护正式说明书和 `.context.md`，输出门禁结论 |
| `sdd-module-asis-analysis` | 需要用代码证据确认现状、边界、隐藏约束、风险和测试覆盖时 | 正式说明书中的 ASIS 关键结论，`.context.md` 中的证据索引和过程 |
| `sdd-module-tobe-design` | 已有 ASIS，需要设计可直接指导 AICoding 的目标方案时 | TOBE 设计、工程落点、接口/数据/配置变化、AICoding 任务和最小验证集 |
| `sdd-module-design-gate` | 需要判断正式说明书是否能进入 AICoding，或门禁不通过后复检时 | 通过/不通过/阻塞结论、四链摘要、阻断问题和整改清单 |
...
- 正式说明书是唯一开发依据。任何会影响编码实现、接口契约、包/类设计、数据库、配置、调用流程、验证方式、风险控制或回退策略的内容，都必须进入正式说明书。
...
sdd-workflow-skills/
  ...
  sdd-module-asis-analysis/
    ...
    references/asis-output-template.md # ASIS 双文件模板
    references/evidence-checklist.md   # 证据挖掘清单
  sdd-module-tobe-design/
    SKILL.md                           # TOBE 阶段规则
    agents/openai.yaml                 # Agent 接口配置
    references/tobe-output-template.md # TOBE 双文件模板
    references/aicoding-task-template.md
  sdd-module-design-gate/
    SKILL.md                           # 门禁阶段规则
    agents/openai.yaml                 # Agent 接口配置
    references/gate-result-template.md # 门禁双文件模板
...
- **context 文件**：`{AR编号}-{需求短名}-{模块名}模块详细设计说明书.context.md`，只记录过程、证据、推导、采样和复检。
- **ASIS**：基于代码证据的现状分析，只确认事实、推断和不确定性，不提前设计 TOBE。
- **TOBE**：基于 ASIS 事实和模块边界的目标设计，必须落到工程位置、任务、验收标准和最小验证集。
- **设计门禁**：判断正式说明书是否能作为唯一开发依据进入 AICoding，并输出整改或阻塞结论。
