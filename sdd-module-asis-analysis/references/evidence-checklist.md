...
- `softWare.md`，包括仓库根目录和子目录中的文件。
- 包、命名空间、模块、工作区、Gradle/Maven/npm/pnpm/yarn/go/cargo/dotnet 工程边界。
- 归属元数据、CODEOWNERS、README、架构文档、ADR。
- 目标模块的 import 关系和反向引用。
...
正文中的重要结论必须引用证据编号，证据编号必须能在当前 SDD 工作流指定的目标成果物证据索引中反查到具体位置和支撑结论；如果工作流未指定目标成果物，则默认在 `xxx模块详细设计说明书` 中维护证据索引。
