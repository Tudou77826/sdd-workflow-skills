...
TOBE 必须基于 `.context.md` 中的 ASIS 证据形成设计，不得脱离证据凭空设计。正式说明书是面向用户、评审和开发的标准交付件，只呈现设计结论、现状约束摘要、接口契约、模块边界、实现方案、风险、验证和待确认事项；不得写入 ASIS 检索过程、证据编号表、推导过程、反证记录、门禁采样、追踪矩阵等过程性内容。`.context.md` 只记录证据、推导和过程，不参与后续开发指导。
...
列出本次设计必须保持或新增的开发契约。下表中的每一项都必须在正式说明书后续章节和 AICoding 任务中可直接执行；不得只写“见上文”“按原定义”“新增接口”等概要。

| 契约编号 | 契约类型 | 名称 / 签名 / Path / Topic | 关键内容 | 适用章节 | AICoding 任务 |
|---|---|---|---|---|---|
| K1 | REST / RPC / MQ / Method / DTO / Field / ErrorCode / Config / SQL |  | method、path、函数签名、入参、出参、字段名、JSON 名称、错误码、状态值、单位、默认值等 | 3 / 7 / 8.1.4 / 8.1.10 | T1 |
